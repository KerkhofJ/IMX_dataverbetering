...
def get_cell_context(ws, row_idx, columns):
    return {
        "Puic": ws.cell(row=row_idx, column=columns["@puic"]).value if columns["@puic"] else None,
        "ObjectPath": ws.cell(row=row_idx, column=columns["path"]).value if columns["path"] else None,
        "ChangeStatus": ws.cell(row=row_idx, column=columns["status"]).value if columns["status"] else None,
        "GeometryStatus": ws.cell(row=row_idx, column=columns["geometry_status"]).value if columns["geometry_status"] else None
    }
...
def handle_header_comment(cell, ws, header_value, columns, sheetname, header_row):
    ...
    if cell.fill.fgColor and cell.fill.fgColor.rgb != "FFFFFFFF":
        for row_idx in range(header_row + 1, ws.max_row + 1):
            ...
            if target_cell.fill.fgColor and target_cell.fill.fgColor.rgb != "FFFFFFFF" and target_cell.value:
...
def write_comments_sheet(wb, comments):
    ...
    ws.append(["ObjectPath", "Puic", "ChangeStatus", "GeometryStatus", "Link", "ImxPath", "Value", "Comment", "Color", "CommentSheetName", "CommentRow", "CommentColumn"])
    ...
    for comment in comments:
        ...
        row = [
            comment["ObjectPath"], comment["Puic"], comment["ChangeStatus"], comment["GeometryStatus"], link,
            comment["Header"], comment["Value"], comment["Comment"], comment["Color"],
            comment["CommentSheetName"], comment["CommentRow"], comment["CommentColumn"]
        ]
        ws.append(row)
        if comment["Color"]:
            ws[f"E{ws.max_row}"].fill = PatternFill(start_color=comment["Color"], end_color=comment["Color"], fill_type="solid")
    ws.freeze_panes = "A2"
    for col in range(1, ws.max_column + 1):
            ...
            for cell in row:
                if cell.comment:
                    ...
                    if cell.row == header_row:
                        all_comments.extend(handle_header_comment(cell, ws, header_value, columns, sheetname, header_row))
                    else:
                        all_comments.extend(handle_data_comment(cell, ws, header_value, columns, sheetname))
