...
def build_manifest(
    ...
    if is_zip:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not output_zip:
                output_zip = input_path.with_stem(
                    input_path.stem + ("-" + datetime.now(timezone.utc).strftime("%Y-%m-%dT%H_%M_%SZ") if include_timestamp else "")
                )
        ...
        if not output_zip:
            timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H_%M_%SZ") if include_timestamp else ""
            output_zip = folder.parent / f"{folder.name}-manifest{('-' + timestamp) if timestamp else ''}.zip"
