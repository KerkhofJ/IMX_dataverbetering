...
def get_revision_template(out_file_path: str | Path):
    ...
    columns = [
        "ObjectPath", "ObjectPuic",
        "IssueComment", "IssueCause",
        "AtributeOrElement", "Operation",
        "ValueOld", "ValueNew",
        "ProcessingStatus", "RevisionReasoning"
    ]
    first_row = [
        "Imx Object Path", "Puic of object for revision",
        "What is the issue", "What is the cause of the issue",
        "The attribute or element path", "Type of revision operation",
        "Old value that is being checked if it is still like this", "Revision Value",
        "Boolean If revision need to be processed", "Revision reasoning, why this value, or why not to revision?"
    ]
    example_row_1 = [
        "dummy.object.path", "a8cfb00e-bbb3-4a83-9783-4f94e013fa9d",
        "Unknown upgrade", "just a example",
        "@name", "UpdateAttribute",
        "Unknown", "SomeOtherValue",
        "True", "Will revision if old value still matches"
    ]

    example_row_2 = [
        "dumy.obkect.path", "a8cfb00e-bbb3-4a83-9783-4f94e013fa9d",
        "Unknown upgrade", "just a example",
        "RailConnectionInfo.@atMeasure", "UpdateAttribute",
        "0", "0.255",
        "False", "Will NOT revision ProcessingStatus = False"
    ]
    ...
    for col_idx in range(1, ws.max_column + 1):  # Loop through columns
        ...
        max_length = max(
            (len(str(cell)) for row in ws.iter_rows(min_col=col_idx, max_col=col_idx, values_only=True) for cell in
             row),
    ...
    operation_values = [
        ...
        "DeleteObject", "AddElementUnder", "DeleteElement"
    ]
    dv = DataValidation(
        type="list",
