...
def test_revision_template_creates_file(output_path: str):
    ...
    result = runner.invoke(
        app, ["revision", "template", f"{output_path}/template.xlsx"]
    )
...
def test_revision_template_fails_on_invalid_extension(output_path: str):
    clear_directory(Path(output_path))
    result = runner.invoke(
        app, ["revision", "template", f"{output_path}/template.not_xlsx"]
    )
    assert result.exit_code == 1, (
        "Should not create template file if file is not .xlsx or .xlsm"
    )
