...
def check_comment_sheet_is_present(out_file: str | Path):
    ...
    actual_headers = [
        cell.value for cell in next(comment_sheet.iter_rows(min_row=1, max_row=1))
    ]
...
def test_extract_comments_to_new_file(diff_report, clean_output_path):
    out_file = Path(clean_output_path) / "comment-list.xlsx"
    extract_comments_to_new_sheet(diff_report, f"{out_file}", overwrite=True)
    assert out_file.exists(), f"Excel file not found at {out_file}"
    check_comment_sheet_is_present(out_file)


def test_extract_comments_to_new_sheet_with_add_to_wb(diff_report, clean_output_path):
    output_file = Path(clean_output_path) / "copied-with-comments.xlsx"
