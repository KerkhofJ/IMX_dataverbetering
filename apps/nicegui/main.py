...
from nicegui import app, context, events, native, run, ui
...
def main_page():
    ...
    with ui.column().classes("items-left justify-left mt-6 w-full"):
        ui.label("ImxInsights.app").classes("text-3xl font-bold")
    ...
    with ui.tab_panels(tabs, value=diff_tab).classes("w-full overflow-hidden"):
        with ui.tab_panel(diff_tab):
            ...
            t1_situation = (
                ui.select(list(ImxSituationEnum.__members__), label="🧾 T1 Situation")
                .props("disable")
                .classes("w-full")
            )
            upload_t1.on(
            ...
            t2_situation = (
                ui.select(list(ImxSituationEnum.__members__), label="🧾 T2 Situation")
                .props("disable")
                .classes("w-full")
            )
            upload_t2.on(
            ...
            async def run_diff():
                ...
                try:
                    ...
                    ui.notify(
                        "✅ Diff completed! Downloading ZIP... 📦", type="positive"
                    )

                    # Clean up work and uploads folders
                    for folder in [
                        HIDDEN_BASE_DIR / client_id / "work",
                        HIDDEN_BASE_DIR / client_id / "uploads",
                    ]:
                        shutil.rmtree(folder, ignore_errors=True)
        ...
        with ui.tab_panel(pop_tab):
            ...
            async def run_population():
                ...
                try:
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    base_dir = HIDDEN_BASE_DIR / client_id / f"work/pop_{timestamp}"
                    zip_dir = HIDDEN_BASE_DIR / client_id
                    base_dir.mkdir(parents=True, exist_ok=True)
                    ...
                    ui.download(zip_path)
                    ui.notify(
                        "✅ Population report completed! Downloading ZIP... 📦",
                        type="positive",
                    )
